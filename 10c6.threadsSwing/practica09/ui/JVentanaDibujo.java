...
public class JVentanaDibujo extends JFrame implements Runnable
{
	...
	public JVentanaDibujo()
	{
		...
		circuloAnimado = new Circulo(10, 550, true, Color.PINK, 40);	
		dibujo.addFigura("Animado", circuloAnimado);
	...
	public void run()
	{
		int xCirculoAnimado = 1;
		int shift = 15;
		circuloAnimado = dibujo.getFirstFigura("Animado");
		while(true)
		{
			try
			{
				circuloAnimado.setX(xCirculoAnimado);
				lienzo.repaint();
				Thread.sleep(100);
				xCirculoAnimado+=shift;
				if(xCirculoAnimado>lienzo.getWidth() || xCirculoAnimado<1)
					shift*=-1;
			}
