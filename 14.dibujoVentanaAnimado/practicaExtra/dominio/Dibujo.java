...
public class Dibujo implements java.io.Serializable
{
	...
	public void checkImpacto(Figura figuraCheck)
	{
		for(Collection<Figura> figuras:figuras.values())
			for(Figura figura:figuras)
				if (figuraCheck != figura)
					if (figuraCheck.checkImpacto(figura) && figura.isVisible() && figuraCheck.isVisible()) {
						figura.setVisible(false);
						figuraCheck.setVisible(false);
					}
	}
}